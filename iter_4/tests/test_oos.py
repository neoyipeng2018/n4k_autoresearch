...
def test_passes_validation_gate_rejects_ruin_negative_validation_and_nonfinite_metrics() -> None:
    base = OOSResult(
        train_cagr=0.10,
    ...
    assert not passes_validation_gate(OOSResult(**(base.__dict__ | {"validation_cagr": -0.01})), 0.0)
    assert not passes_validation_gate(OOSResult(**(base.__dict__ | {"validation_cagr": float("nan")})), 0.0)
    assert passes_validation_gate(OOSResult(**(base.__dict__ | {"excess_oos_cagr_vs_dbmf": -0.01})), 0.0)
