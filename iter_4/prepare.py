...
from datasets.cache import MARKET_DIR
from datasets.loaders import MissingPublicDataError
from datasets.public_features import load_public_feature_frame
...
class EvaluationSplit:
    ...
    role: Literal["warmup", "train", "validation", "oos"]
...
DEFAULT_SPLITS: tuple[EvaluationSplit, ...] = (
    EvaluationSplit("warmup", "2018-01-01", "2019-12-31", "warmup"),
...
def add_public_features(prices: pd.DataFrame) -> pd.DataFrame:
    try:
        features = load_public_feature_frame(prices)
    except MissingPublicDataError:
        return prices
    if features.empty:
        return prices
    numeric_features = features.select_dtypes(include=["number"])
    return prices.join(numeric_features, how="left")
...
def load_prices(symbols: list[str], start: str = DEFAULT_DATA_START) -> pd.DataFrame:
    ...
    return add_public_features(prices)
...
def daily_returns(prices: pd.DataFrame) -> pd.DataFrame:
    return prices.pct_change(fill_method=None).replace([math.inf, -math.inf], pd.NA)
...
def prices_for_split(prices: pd.DataFrame, split: EvaluationSplit) -> pd.DataFrame:
    start = pd.Timestamp(split.start)
    frame = prices.loc[prices.index >= start]
    if split.end is not None:
        frame = frame.loc[frame.index <= pd.Timestamp(split.end)]
    if len(frame.index) < 2:
        raise RuntimeError(f"Split {split.name} has insufficient observations")
    return frame


def split_by_role(splits: tuple[EvaluationSplit, ...], role: Literal["train", "validation", "oos"]) -> EvaluationSplit:
    matches = [split for split in splits if split.role == role]
    if len(matches) != 1:
        raise ValueError(f"Expected exactly one {role} split")
    return matches[0]
...
def evaluate_oos(
    ...
    universe = DEFAULT_UNIVERSE if symbols is None else symbols
    market_prices = load_prices(universe, start=DEFAULT_DATA_START) if prices is None else prices.copy()
    market_prices = market_prices.sort_index()
    train = run_backtest(
        select_weights,
        symbols=universe,
        prices=prices_for_split(market_prices, split_by_role(splits, "train")),
    )
    validation = run_backtest(
        select_weights,
        symbols=universe,
        prices=prices_for_split(market_prices, split_by_role(splits, "validation")),
    )
    oos = run_backtest(
        select_weights,
        symbols=universe,
        prices=prices_for_split(market_prices, split_by_role(splits, "oos")),
    )
    ruined = train.ruined or validation.ruined or oos.ruined
    result = OOSResult(
        train_cagr=train.cagr,
        validation_cagr=validation.cagr,
        oos_cagr=oos.cagr,
        benchmark_oos_cagr=oos.dbmf_cagr,
        excess_oos_cagr_vs_dbmf=oos.excess_cagr_vs_dbmf,
        ruined=ruined,
        passed=False,
    )
    return OOSResult(
...
def passes_validation_gate(result: OOSResult, previous_best_validation_cagr: float) -> bool:
    if result.ruined:
        return False
    if not all(
        math.isfinite(value)
