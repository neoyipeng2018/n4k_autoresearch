...
def evaluate_oos(
    ...
    previous_best_oos_cagr: float = 0.0,
) -> OOSResult:
    universe = DEFAULT_UNIVERSE if symbols is None else symbols
    ...
    return OOSResult(
        ...
        passed=passes_oos_gate(result, previous_best_oos_cagr),
    )
...
def passes_oos_gate(result: OOSResult, previous_best_oos_cagr: float) -> bool:
    ...
    if not all(
        ...
        return False
    if result.oos_cagr <= previous_best_oos_cagr:
        return False
    if result.excess_oos_cagr_vs_dbmf <= 0.0:
        return False
    if result.validation_cagr <= 0.0:
        return False
    if result.train_cagr > 0.0 and result.oos_cagr < 0.25 * result.train_cagr:
        return False
    return True
